...
/*
    function authorizeRequestBerthing(uint id) public onlyBy(application){
        require(msg.sender == application, "Only application can approve a request");
        registries[id].status = Status.AUTHORIZED ;
    }

 

    function getLastRegistry() public view returns (BerthingForm memory){
        uint last = registries.length - 1;
        return registries[last];
       
    }

    function getSpecificRegistry(uint id) public view returns (BerthingForm memory){
       return registries[id];
    }
   
    function getAllRegistry() public view returns (BerthingForm[] memory){
        return registries;
    }


    function isManager() public view returns (string memory ){
        require(msg.sender == process, "only manager can call this method"); 
        return string(abi.encodePacked("process",": ",toAsciiString(msg.sender)));    
    }
    ...
    // ------------ converter msg.sender em string ----------------
    ...
    // ------------ FIM : converter msg.sender em string ----------------
*/
